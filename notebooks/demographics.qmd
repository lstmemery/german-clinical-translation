...
df %>% 
  count(Age_yrs) %>% 
  arrange(Age_yrs) %>%
  write_csv("../results/count_age.csv")
```
...
```{r}
gender_df %>% 
  write_feather("../results/gender.feather")
```
...
df %>% 
  ...
  write_csv("../results/count_gender.csv")
```
...
df %>%
  ...
  select(-`T1_PAT_1.1`) %>% 
  write_feather("../results/school_level.feather")
```
...
df %>% 
  ...
  write_csv("../results/count_school_level.csv")
```
...
df %>% 
  ...
  select(-`T1_PAT_1.2`) %>% 
  write_feather("../results/professional_level.feather")
```
...
df %>% 
  ...
  write_csv("../results/count_professional_level.csv")
```
...
df %>% 
  ...
  write_csv("../results/count_hospitalization_length.csv")
```
...
df %>% 
  ...
  write_csv("../results/count_migration_background.csv")
```
...
df %>% 
  ...
  write_csv("../results/count_motivation.csv")
...
```{r}
write_csv(primary_conditions_df, "../results/count_primary_conditions.csv")
```
...
```{r}
write_csv(secondary_conditions_df, "../results/count_secondary_conditions.csv")
```
